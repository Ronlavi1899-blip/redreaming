...
/* ── הבטחות שאין מאחוריהן מימוש ── */

test('3ל. אין הבטחה לשליחת החומרים במייל - אין עדיין אוטומציה כזו', () => {
  // נכון ל-2 באוגוסט 2026 אין שום מנגנון ששולח מייל אחרי רכישה.
  // PayPlus שולחת חשבונית, וזה הכל. כל טקסט שמבטיח קישור במייל
  // יוצר ציפייה שלא מתקיימת אצל לקוח משלם.
  const promises = [
    /שלחתי לך.{0,20}מייל/,
    /אשלח לך.{0,20}מייל/,
    /יישלח.{0,20}מייל/,
    /לשמור את המייל/,
    /קידומי מכירות|ספאם/,      // הופיע רק בהקשר של "אם המייל לא הגיע"
  ];

  for (const page of ['thank-you.html', 'start.html']) {
    const html = readFileSync(join(ROOT, page), 'utf8');
    for (const pattern of promises) {
      assert.ok(!pattern.test(html),
        `${page} מבטיח מייל שלא נשלח בפועל: ${pattern}`);
    }
  }
});
